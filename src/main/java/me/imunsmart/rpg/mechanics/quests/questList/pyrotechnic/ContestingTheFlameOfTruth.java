...
public class ContestingTheFlameOfTruth extends Quest {
    ...
    public ContestingTheFlameOfTruth(Main pl) {
        ...
        torch = Items.createItem(Material.REDSTONE_TORCH_ON, 1, 0, ChatColor.RED + "Red Hot Stick", "A mysterious fiery stick...", "The Pyrotechnic might be interested in this item.", "", ChatColor.YELLOW + getName() + " Quest Item");
        torch.addUnsafeEnchantment(Enchantment.getByName("glow"), 1);
        firespirit = Items.createItem(Material.TOTEM, 1, 0, ChatColor.RED + "Spirit of Fire", "A poor, cursed flame soul.", "The Pyrotechnic would be interested in this item.", "", ChatColor.YELLOW + getName() + " Quest Item");
