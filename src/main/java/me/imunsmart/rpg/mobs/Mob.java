...
public class Mob {
	...
	private String name;
	private UUID lastPlayerToHit = null;
	...
	public Mob(UUID mob, String name, int tier) {
		...
		// Wear Skull
		getMob().getEquipment().setHelmet(new PlayerSkull(Util.names[(int) (Math.random() * Util.names.length)]));
	...
	public Mob(UUID mob, String name, int tier, String type, int minDMG, int maxDMG, String weaponFlag, int maxHelmet, int maxChestplate, int maxLeggings,
		...
		if (getMob().getType() == EntityType.ZOMBIE || getMob().getType() == EntityType.SKELETON) {
			...
			getMob().getEquipment().setItemInMainHandDropChance(0);
			// Wear Skull
			getMob().getEquipment().setHelmet(new PlayerSkull(skullName));
		}
		maxHP = health = Health.calculateMaxHealth(getMob()) + Health.getAttribute(b, "Health");
		getMob().setCustomName(name);
		getMob().setCustomNameVisible(true);
	...
	public void damage(double i, Player p) {
		RPGMobDamagedEvent e = new RPGMobDamagedEvent(p, this);
		Bukkit.getPluginManager().callEvent(e);
		
		if (!e.isCancelled()) {
			lastPlayerToHit = p.getUniqueId();
			hitsTaken++;
	...
	private void die() {
		RPGMobKilledEvent e = new RPGMobKilledEvent(lastPlayerToHit, this);
		Bukkit.getPluginManager().callEvent(e);
	...
	public void invalidateDrop(boolean helmet, boolean chestplate, boolean leggings, boolean boots, boolean weapon) {
		if (drop == null)
			return;
		else if (drop.getType().name().contains("HELMET") && helmet)
			drop = null;
		else if (drop.getType().name().contains("CHESTPLATE") && chestplate)
			drop = null;
		else if (drop.getType().name().contains("LEGGINGS") && leggings)
			drop = null;
		else if (drop.getType().name().contains("BOOTS") && boots)
			drop = null;
		else if (drop.getType().name().contains("WEAPON") && weapon)
			drop = null;
	}
}
