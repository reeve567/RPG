...
public class ItemManager {
	...
	public static int getMinDamage(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("MIN_DAMAGE_" + tier);
	...
	public static int getMaxDamage(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("MAX_DAMAGE_" + tier);
			return f.getInt(null);
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
	...
	public static int getMaxHealth(int tier, String i) {
		try {
			Field f = ItemManager.class.getDeclaredField("MAX_HEALTH_" + tier + i);
			return f.getInt(null);
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
	...
	public static double getMaxRegen(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("MAX_REGEN_" + tier);
	...
	public static double getMaxDrops(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("MAX_T" + tier + "_GEMS");
			return f.getDouble(null);
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
		} catch (IllegalAccessException e) {
	...
	public static String getRandomZombieName(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("zt" + tier);
	...
	public static String getRandomSkeletonName(int tier) {
		try {
			Field f = ItemManager.class.getDeclaredField("st" + tier);
			String[] name = (String[]) f.get(null);
			return name[(int) (Math.random() * name.length)];
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
	...
	public static String randomArmorFlag(int max, int tier) {
		String flag = "";
		if (Math.random() < 0.6) {
			flag += "Regen:" + (int) (max * ItemManager.getMaxRegen(tier));
		}
